...
public class EditAccountService {
     ...
     */
    public Map<String, Object> getAccountDetails(String userId, String accountTypeId) {
        if (dbConn == null) {
        ...
        String selectQuery = "SELECT ua.user_id, ua.account_type_id, ua.initial_balance, ua.current_balance, " +
                            ...
                            "WHERE ua.user_id = ? AND ua.account_type_id = ?";
     ...
     *
     * @param userId           the ID of the user
     * @param accountTypeId    the current ID of the account type
     * @param newAccountTypeId the new ID of the account type
     * @param nickname         the new nickname for the account
     * @return Boolean indicating the success of the operation
     */
    public Boolean updateAccount(String userId, String accountTypeId, String newAccountTypeId, String nickname) {
        if (dbConn == null) {
            System.err.println("Database connection is not available.");
            return false;
        }

        String updateQuery = "UPDATE user_accounts SET account_type_id = ?, nickname = ? " +
                            "WHERE user_id = ? AND account_type_id = ?";

        try (PreparedStatement stmt = dbConn.prepareStatement(updateQuery)) {
            stmt.setString(1, newAccountTypeId);
            stmt.setString(2, nickname);
            stmt.setString(3, userId);
            stmt.setString(4, accountTypeId);
     ...
     * @return Boolean indicating the success of the operation
     */
    public Boolean closeAccount(String userId, String accountTypeId) {
        if (dbConn == null) {
            System.err.println("Database connection is not available.");
            return false;
        }
